...
-   Ahora, quitamos las entidades con demasiado ruido en el periodo pretratamiento:

    -   Se quitan Baja California, Baja California Sur, Chihuahua, Colima, Guanajuato, Morelos, Nayarit, Quintana Roo.

-   También eliminamos 3 entidades con programas de canjes de armas de fuego ligeramente desarollados (Jalisco, México y Zacatecas).

-   **Es decir, nos quedamos con 20 unidades de control y 1 de tratamiento.**
...
## Control Sintético 2. Suavizado por Moving Averages

-   Se aplica una suavización de promedio móvil de 3 periodos (meses)
...
-   Filtro de variabilidad:

```{r}
# Calcular la variabilidad de cada unidad antes del tratamiento
variabilidad <- delitosCS_smooth_ma %>%
  filter(Fecha >= pre_treatment_period[1] & Fecha <= pre_treatment_period[2]) %>%
  group_by(Entidad) %>%
  summarise(variabilidad = sd(armas_Cuenta, na.rm = TRUE))
...
# Filtrar el dataframe original para incluir solo unidades estables
delitosCS_filtrado <- delitosCS_smooth_ma %>%
  filter(Entidad %in% unidades_estables | Entidad == "Ciudad de México") # Asegurarse de incluir la unidad tratada
```
...
principalCSdelitos %>%
  ...
  geom_line(aes(time_unit, armas), size = 1, color = "darkgray") +
  theme_clean() +
  labs(title="Delitos con armas de fuego por cada 100 000 habitantes, \n CDMX observada vs. CDMX sintética vs. CDMX sin DSE. Suavizada por MA.",
       x = "Fecha", y = "Incidencia", caption = "\n Fuente: Elaboración propia con datos del SESNSP y Sedena. \n \n Nota: La línea punteada vertical indica el inicio del programa 'Sí al desarme, sí a la paz'.") +
  scale_x_date(limits = c(as.Date("2017-01-01"), as.Date("2022-12-01")),
               breaks = seq(as.Date("2017-01-01"), as.Date("2022-12-01"), by = "3 months"), 
               labels = scales::date_format("%m/%Y")) +
...
-   p-value para efecto de tratamiento:
...
-   Placebos con las otras 20 unidades de control:
...
## Control Sintético 3. Suavizado por LOESS

-   Se realiza una suavización LOESS a los datos, que implica calcular regresiones locales en ventanas pequeñas de tiempo. Esto se hizo con base en los datos originales de incidencia por cada 100,000 habitantes.
...
principalCSdelitos %>%
  ...
  geom_line(aes(time_unit, armas), size = 1, color = "darkgray") +
  theme_clean() +
  labs(title="Delitos con armas de fuego por cada 100 000 habitantes, \n CDMX observada vs. CDMX sintética vs. CDMX sin DSE. Suavizada por LOESS",
       x = "Fecha", y = "Incidencia", caption = "\n Fuente: Elaboración propia con datos del SESNSP y Sedena. \n \n Nota: La línea punteada vertical indica el inicio del programa 'Sí al desarme, sí a la paz'.") +
  scale_x_date(limits = c(as.Date("2017-01-01"), as.Date("2022-12-01")),
               breaks = seq(as.Date("2017-01-01"), as.Date("2022-12-01"), by = "3 months"), 
               labels = scales::date_format("%m/%Y")) +
...
-   Placebos para las 20 unidades de control:
...
CSplacebos %>%
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 14,face="bold"), axis.title=element_text(size=12,face="bold"),
        axis.text.x = element_text(angle = 65, hjust =1), plot.caption = element_text(hjust = 0))+
  geom_line(data = filter(CSplacebos, Entidad == "Ciudad de México"), size = 1, color = "#d0ce71") +
  annotate("text", x=as.Date("2021-01-01"), y=-2.1, label= "CDMX", size = 4, fontface = "bold", color = "#d0ce71") +
  geom_vline(xintercept=as.Date("2019-01-01"), linetype="dashed", size = 1.1)+
  geom_hline(yintercept=0, linetype="dashed", size = 1.1)
```
